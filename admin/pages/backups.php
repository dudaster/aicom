...
<div class="wrap wpops-wrap">
    <h1>Backups <span class="wpops-count">(<?php echo number_format( $total ); ?> total)</span></h1>
    ...
    <form method="get" action="" class="wpops-filters">
        <input type="hidden" name="page" value="wpops-mcp-backups" />
        <div class="wpops-filter-row">
            ...
            <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpops-mcp-backups' ) ); ?>" class="button">Reset</a>
        </div>
    </form>
            ...
            <tr>
                ...
                <td>
                    <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                        ...
                        <input type="hidden" name="wpops_action" value="delete_backup" />
                        <input type="hidden" name="backup_id" value="<?php echo (int) $backup['id']; ?>" />
                        <?php wp_nonce_field( WPOPS_Admin::NONCE_ACTION ); ?>
                        <button type="submit" class="button button-small wpops-btn-danger" onclick="return confirm('Delete this backup? This cannot be undone.')">Delete</button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
    ...
    <div class="tablenav bottom">
        <div class="tablenav-pages">
            <?php for ( $p = 1; $p <= $num_pages; $p++ ) :
                $url = admin_url( 'admin.php?page=wpops-mcp-backups&target_type=' . urlencode( $filter_type ) . '&paged=' . $p );
