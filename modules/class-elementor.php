 ...
 */
class WPOPS_Module_Elementor extends WPOPS_Module_Base {
    ...
    public function handle_restore( array $args, array $key_record, bool $dry_run ): array {
        ...
        $backup = $wpdb->get_row(
            $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}wpops_mcp_backups WHERE id = %d AND target_type = 'elementor_page'", $backup_id ),
            ARRAY_A
        );
